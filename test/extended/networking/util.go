...
func skipUnsupportedControlPlaneTopology(oc *exutil.CLI) {
	...
	if err != nil || controlPlane == nil {
		e2e.Logf("Failed : failed to get control plane topology, errored(%w) or controlPlane was nil\n)", err)
		return
	}

	switch *controlPlane {
	case configv1.DualReplicaTopologyMode, configv1.HighlyAvailableArbiterMode:
		e2eskipper.Skipf("Unsupported control plane topology, skipping (%v out of %v)", controlPlane)
	}
}
